# Rock Paper Scissors

A classic Rock Paper Scissors game played in the browser — built as part of my JavaScript learning journey.

## 📖 About

A simple game of rock-paper-scissors with a simple UI in a browser, featuring a scoreboard and rounds.

---

## 🔗 Live Demo

[Click here to view the live demo](https://ikram-muhr.github.io/odin-foundations-course/03-rock-paper-scissors/)

---

## 🚀 How to Run

1. Clone this repository

    ```bash
   git clone https://github.com/ikram-muhr/odin-foundations-course.git
    ```

2. Navigate to the project folder

    ```bash
   cd odin-foundations-course/03-rock-paper-scissors
    ```

3. Open `index.html` in your browser
4. Start the game by pressing the rock-paper-scissors button

---

## 🛠️ Built With

- HTML5
- CSS3
- Javascript

---

## 💡 What I’ve Learned

- How to create logic in JavaScript using:
  - Functions
  - `switch` statements
  - Logical operators & comparisons
  - `if / else` statements
- How to generate random values with `Math.random()`
- How to keep track of state with variables scoped inside a function

## 🤔 The Challenges I Face

- How to make code easier to read. I tried adding some comments to the code and organizing it more neatly.
- How to create the logic for this rock-paper-scissors game. I used a combination of if statements and switch statements to make the code more readable and concise.

---

## 🚧 What I Want to Improve

- Add more interactive animations for each element that changes

---

## 📎 Source

- [The Odin Project](https://www.theodinproject.com/lessons/foundations-rock-paper-scissors)
